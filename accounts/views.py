...
def address(request):
    ...
    saved_address = Billing.objects.filter(user_id = register_id)
    ...
    if request.method == 'POST':
        ...
        if selected_address:
            try:
                address = Billing.objects.get(id = selected_address)
        ...
        if payment_method == 'COD':
            ...
            session = stripe.checkout.Session.create(
                ...
                line_items= [{
                    'price_data' : {
                        'currency':'inr' ,
                        'product_data' : {'name': 'Order from Nextron'},
...
def payment_success(request, order_id=None):
    ...
    try:
        billing = Billing.objects.get(id=address)
    except Billing.DoesNotExist:
